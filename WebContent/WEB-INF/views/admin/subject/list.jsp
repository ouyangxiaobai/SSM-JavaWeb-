	...
	$(function() {	
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        columns: [[  
				{field:'chk',checkbox: true,width:50},
 		        ...
 		        {field:'type',title:'主题类型',width:200,formatter: function(value,row,index){
 		        	return voteTypeJson[value];
 		        }},
 		        {field:'info',title:'主题介绍',width:300},
	    ...
	    $("#delete").click(function(){
        	...
        	if(selectRow == null){
            	...
            	$.messager.confirm("消息提醒", "若在改主题下存在投票选项则无法删除，确认继续？", function(r){
            		if(r){
            			$.ajax({
							type: "post",
							url: "delete",
	  ...
	  //设置编辑班级窗口
	    $("#editDialog").dialog({
			...
			onBeforeOpen: function(){
				...
				$("#edit_start_time").datetimebox('setValue', formatterDate(selectRow.startTime));
				$("#edit_end_time").datetimebox('setValue', formatterDate(selectRow.endTime));
			}
	    });
    	...
    	<form id="addForm" method="post">
	    	<table cellpadding="8" >
	    		...
	    		<tr>
	    			<td>主题类型:</td>
	    			<td>
		    			<select style="width: 200px; height: 30px;" class="easyui-combobox" name="type"  data-options="required:true, missingMessage:'不能为空'" />
		    			<c:forEach items="${ voteType}" var="vote_type">
		    				<option value="${ vote_type.name()}">${vote_type.tips}</option>
		    			</c:forEach>
		    			</select>
	    			</td>
	    		</tr>
	    		<tr>
	    			<td>开始时间:</td>
	    			<td><input id="add_start_time" style="width: 200px; height: 30px;" class="easyui-datetimebox" type="text" name="startTime"  data-options="required:true, missingMessage:'不能为空'" /></td>
	    		</tr>
	    		<tr>
	    			<td>结束时间:</td>
    	...
    	<input type="hidden" id="edit-id" name="id">
	    	<table cellpadding="8" >
	    		...
	    		<tr>
	    			<td>主题类型:</td>
	    			<td>
		    			<select id="edit_type" style="width: 200px; height: 30px;" class="easyui-combobox" name="type"  data-options="required:true, missingMessage:'不能为空'" />
		    			<c:forEach items="${ voteType}" var="vote_type">
		    				<option value="${ vote_type.name()}">${vote_type.tips}</option>
		    			</c:forEach>
		    			</select>
	    		...
	    		<tr>
	    			<td>开始时间:</td>
	    			<td><input id="edit_start_time" style="width: 200px; height: 30px;" class="easyui-datetimebox" type="text" name="startTime"  data-options="required:true, missingMessage:'不能为空'" /></td>
	    		</tr>
	    		<tr>
	    			<td>结束时间:</td>
	    			<td><input id="edit_end_time" style="width: 200px; height: 30px;" class="easyui-datetimebox" type="text" name="endTime"  data-options="required:true, missingMessage:'不能为空'" /></td>
	    		</tr>
	    		<tr>
	    			<td>主题介绍:</td>
	    			<td>
	    				<textarea id="edit_info" name="info" style="width: 260px; height: 160px;" class="" ></textarea>
	    			</td>
	    		</tr>
	    	</table>
	    </form>
